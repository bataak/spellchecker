...
test("skips drawings, text boxes, field instructions and deleted text", () => {
  const xml = doc(
    "<w:p>" +
      run("эхлэл") +
  ...
  assert.equal(textOf(xml), "эхлэл12 төгсгөл");
});
...
test("replaces a word split across runs with identical formatting", () => {
  ...
  const xml = doc(
    `<w:p>${run("хар", bold)}${run("ъяалал", bold)}${run(" нь", bold)}</w:p>`,
  );
...
test("reports overlapping edits instead of corrupting text", () => {
  const xml = doc(`<w:p>${run("монгол улс")}</w:p>`);
  const part = parsePart("word/document.xml", xml);
  const plan = planParagraph(part.paragraphs[0], [
    edit(0, 6, "Монгол"),
    edit(3, 10, "x"),
  ]);
...
test("reads paragraphs inside table cells", () => {
  const xml = doc(
    `<w:tbl><w:tr><w:tc><w:p>${run("нүд")}</w:p></w:tc></w:tr></w:tbl>`,
  );
  const part = parsePart("word/document.xml", xml);
...
test("records the paragraph style", () => {
  const xml = doc(
    `<w:p><w:pPr><w:pStyle w:val="Heading1"/></w:pPr>${run("Гарчиг")}</w:p>`,
  );
  const part = parsePart("word/document.xml", xml);
...
test("indexes paragraphs across parts with one newline between them", () => {
  const body = parsePart(
    "word/document.xml",
    doc(`<w:p>${run("нэг")}</w:p><w:p>${run("хоёр")}</w:p>`),
  );
  const header = parsePart(
    "word/header1.xml",
    doc(`<w:p>${run("толгой")}</w:p>`),
  );
...
test("renders pending edits onto the flat text", () => {
  const body = parsePart(
    "word/document.xml",
    doc(`<w:p>${run("монгол улс")}</w:p>`),
  );
  const index = buildIndex([body]);

  assert.equal(
    renderText(index, [{ start: 0, end: 6, text: "Монгол" }]),
    "Монгол улс",
  );
});
...
test("leaves a soft hyphen alone when the edit does not reach it", () => {
  const xml = doc(
    `<w:p>${run("симу")}<w:r><w:softHyphen/></w:r>${run("ляци нь")}</w:p>`,
  );
...
test("keeps the visible result of a hyperlink field", () => {
  const xml = doc(
    "<w:p>" +
      run("нарын „") +
      '<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
      '<w:r><w:instrText xml:space="preserve"> HYPERLINK "https://a.example" </w:instrText></w:r>' +
      '<w:r><w:fldChar w:fldCharType="separate"/></w:r>' +
      run("Монгол үсгийн дүрмийн толь") +
      '<w:r><w:fldChar w:fldCharType="end"/></w:r>' +
      run("“-оос") +
      "</w:p>",
  );

  assert.equal(textOf(xml), "нарын „Монгол үсгийн дүрмийн толь“-оос");
});

test("drops the field instruction but keeps a page number result", () => {
  const xml = doc(
    "<w:p>" +
      run("Хуудас ") +
      '<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
  ...
  assert.equal(textOf(xml), "Хуудас 7");
});

test("skips a field that has no result", () => {
  const xml = doc(
    "<w:p>" +
      run("эхлэл ") +
      '<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
...
test("replaces a word inside a hyperlink field result", () => {
  const xml = doc(
    "<w:p>" +
      '<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
      '<w:r><w:instrText> HYPERLINK "https://a.example" </w:instrText></w:r>' +
      '<w:r><w:fldChar w:fldCharType="separate"/></w:r>' +
      run("монгол толь") +
      '<w:r><w:fldChar w:fldCharType="end"/></w:r>' +
      "</w:p>",
  ...
  assert.equal(textOf(out), "Монгол толь");
  assert.ok(out.includes('HYPERLINK "https://a.example"'));
});

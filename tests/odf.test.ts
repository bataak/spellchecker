...
test("joins text across spans", () => {
  const part = partOf(
    doc(
      '<text:p>мон<text:span text:style-name="T1">гол</text:span> улс</text:p>',
    ),
  );
  assert.equal(part.paragraphs[0].text, "монгол улс");
});
...
test("expands text:s into spaces and text:tab into a tab", () => {
  const part = partOf(
    doc('<text:p>а<text:s text:c="3"/>б<text:tab/>в</text:p>'),
  );
...
test("ignores a nested paragraph instead of corrupting the outer one", () => {
  const part = partOf(
    doc(
      "<text:p>гадна<draw:frame><draw:text-box><text:p>дотор</text:p></draw:text-box></draw:frame> үг</text:p>",
    ),
  );
  assert.equal(part.paragraphs.length, 1);

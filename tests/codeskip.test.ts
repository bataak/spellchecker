...
test("skips html code elements", () => {
  const text =
    "Үзүүлбэр <code>getElementById</code> ба <pre>npm test</pre> дуусав.";
...
test("skips a markdown link target but keeps the label", () => {
  const text =
    "Толь нь [Hunspell](http://hunspell.github.io) дээр тулгуурлана.";
...
test("keeps label words that also appear in the url", () => {
  const text =
    "[LibreOffice extensions](https://extensions.libreoffice.org/en/show/1980)";
  const ranges = skipRanges(text);
...
test("skips bare urls, www hosts and autolinks", () => {
  const text =
    "Үз https://zuv.bichig.dev/core/ бас www.bichig.dev мөн <https://a.example.org/x>.";
  const ranges = skipRanges(text);
...
test("keeps an ordinary link label that only looks technical", () => {
  const text =
    "[Sublime Text](https://packagecontrol.io/packages/Dictionaries)";
  const ranges = skipRanges(text);
